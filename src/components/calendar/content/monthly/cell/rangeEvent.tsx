...
const RangeEvent: React.FC<IProps> = ({ index, item, cellIndexInWeek, date, cellWith, setParentHeight }) => {
  ...
  const calcStyleForRange = (): React.CSSProperties => {
    ...
    if (ref.current !== null) {
      const style = {
        ...
        top: (ref.current?.offsetHeight + 5) * (item?.priority !== undefined ? item.priority : index),
      }

      setParentHeight && setParentHeight(((ref.current.offsetHeight + 10 + style.top) / window.innerHeight) * 100)
      return style
    }
  ...
  return (
    <div
      ...
      className={`__calendar-table-td-body-events-item ${item?.className} ${hide ? 'hide' : ''} ${classEvent}`}
    ...
    >
      {!item.icon ? (
        <div className={`__calendar-table-td-body-events-item-circle`} style={{ background: item.dotColor }} />
      ) : (
        item.icon
      )}
      {item.title}
