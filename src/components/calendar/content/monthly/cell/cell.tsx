...
const Cell = ({ date, disabled, onClick, cellIndexInWeek }: IProps) => {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    config.setEvents((prev: IEventLogic[]) => {
      ...
      if (find !== -1) {
        ...
        eventToMove.date.end = moment(newStartDate).locale('en').add(rangeDuration, 'days').format('YYYY-MM-DD')

        if (eventToMove.date.end !== 'Invalid date' && eventToMove.date.start !== 'Invalid date') {
  ...
  const onThisDay = config.onDay ? config.onDay(moment_(date).locale('en').format('YYYY-MM-DD')) : {}
  ...
  return (
    <td
      ...
      onClick={() => {
        if (onClick) onClick()
        if (config.onDateClick) config.onDateClick(moment_(date).locale('en').format('YYYY-MM-DD'))
      }}
    ...
    >
      <div className="__calendar-table-td-body">
        <div className={`__calendar-table-td-body-date`}>{moment_(date).format('DD MMMM')}</div>
