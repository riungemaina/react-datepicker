...
const FillEndAndStart = ({ start, onNext, onPrev }: IPropsFillEndAndStart) => {
  ...
  return (
    <>
      {new Array(start ? getMonthStartWith() : getMonthCountToEnd()).fill('DefaultValue').map((i, index) => (
        <Day onClick={start ? onPrev : onNext} disabled={true} date={date} day={day(index)} key={index} />
      ))}
    </>
  )
}
