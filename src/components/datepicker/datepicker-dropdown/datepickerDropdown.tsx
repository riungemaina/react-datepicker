...
const DatepickerDropdown = ({ open, footer, loading, spinnerComponent, refMain, adjustPosition, setOpen }: IProps) => {
  ...
  return (
    <>
      {adjustPosition === 'modal' && <div className={`__datepicker-modal-bg`} onClick={() => setOpen(false)} />}
      <div
        className={`__datepicker-dropdown ${open ? '__datepicker-dropdown-active' : ''} ${
          adjustPosition !== 'auto' ? (adjustPosition === 'modal' ? '__datepicker-modal' : adjustPosition) : position
        } ${fix ? fix : ''}`}
      >
        <Loading loading={loading} spinnerComponent={spinnerComponent}>
          <>
            ...
            {footer && <div className={`__datepicker-dropdown-footer`}>{footer(moment, setValue)}</div>}
          </>
        </Loading>
      </div>
    </>
